...
export class ProductStore {
    async create(p: Product): Promise<Product> {
        try {
            ...
            const sql =
                "INSERT INTO product_details (product_id, product_name, product_price, product_category) VALUES ($1, $2, $3, $4) RETURNING *";
    ...
    async read(pid: number): Promise<Product> {
        try {
            ...
            throw new Error(
                "Failed to load product details of product with ID: " +
                    pid +
                    " because, " +
                    error
            );
    ...
    async update(id: number, p: Product): Promise<Product> {
        try {
            ...
            throw new Error(
                "Failed to update details of product: " +
                    p.product_name +
                    " because, " +
                    error
            );
    ...
    async delete(pid: number): Promise<Product> {
        try {
            ...
            throw new Error(
                "Product with ID: " + pid + "can't be deleted because, " + error
            );
        }
    }
